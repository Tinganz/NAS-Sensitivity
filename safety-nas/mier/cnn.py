...
# ── output path ───────────────────────────────────────────────────────────────
# NAS trial logs (nas_trials_*.jsonl) and temporary model checkpoints go here.
# Change this if you want a different root, e.g. a cluster scratch drive.
OUTPUT_DIR = BASE_DIR / "output" / "search"
...
# Evaluate on 3 diverse tracks during search to reduce single-track variance.
TRAIN_EVAL_TRACKS = [
...
def _build_training_config(
    ...
    cfg: dict[str, any] = {
        ...
        "training": {
            "max_epochs": 60,   # increased from 30 for better NAS fitness signal
            "lr": 5e-5,
            "weight_decay": 1e-4,
...
def objective(
    ...
    abs_overflow = 0
    total_overflow = 0
    lower_bound = 2000   # tightened from 10000: penalise models more than 2000 params under budget
